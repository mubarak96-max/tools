...
import { reverseText, type ReverseTextMode } from "@/lib/tools/reverse-text";
...
const modes: Array<{ value: ReverseTextMode; label: string; sample: string }> = [
  { value: "full", label: "Reverse text", sample: "hello -> olleh" },
  { value: "word-order", label: "Reverse word order", sample: "one two three -> three two one" },
  { value: "mirror-words", label: "Reverse each word", sample: "hello world -> olleh dlrow" },
];

export default function ReverseTextGenerator() {
  const [text, setText] = useState("");
  const [mode, setMode] = useState<ReverseTextMode>("full");
  const [copyState, setCopyState] = useState<"idle" | "copied" | "error">("idle");

  const result = useMemo(() => reverseText(text, mode), [mode, text]);
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-5">
          <div className="grid gap-4 lg:grid-cols-2">
            <label className="block space-y-2">
              ...
              <textarea
                ...
                placeholder="Paste or type text to reverse it in different ways."
                className={textareaClass}
              />
            </label>

            <label className="block space-y-2">
              <span className="text-sm font-medium text-muted-foreground">Reversed output</span>
              <textarea
                value={result.output}
                readOnly
          ...
          <div className="rounded-[1.5rem] border border-border bg-background p-5">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold text-foreground">Reverse modes</h2>
        ...
        <aside className="space-y-4 rounded-[1.5rem] border border-border bg-background p-5">
          <div>
            ...
            <h2 className="mt-2 text-lg font-semibold text-foreground">Reverse snapshot</h2>
          </div>
          ...
          <div className="grid gap-3">
            ...
            <div className="rounded-[1rem] border border-border bg-card p-4">
              ...
              <p className="mt-2 text-3xl font-semibold tracking-tight text-foreground">{result.words}</p>
            </div>
          ...
          <div className="rounded-[1rem] border border-primary/15 bg-primary-soft p-4">
            <p className="text-sm leading-6 text-primary-soft-foreground">
              Use full reverse for mirrored text effects, word-order reverse for playful copy transformations, and per-word reverse when you want each word flipped in place.
            </p>
          </div>
        </aside>
      </div>
