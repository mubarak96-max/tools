...
import { 
  Gauge, ArrowRightLeft, Download, Info, Link2, Wind
} from "lucide-react";

type UnitOption = "ms" | "kmh" | "mph" | "kn" | "fts" | "mach";

const UNITS: Record<UnitOption, { name: string; symbol: string; toMs: number; type: "metric" | "imperial" | "aero" }> = {
  ms: { name: "Meters / Second", symbol: "m/s", toMs: 1, type: "metric" },
  kmh: { name: "Kilometers / Hour", symbol: "km/h", toMs: 0.277777778, type: "metric" },
  mph: { name: "Miles / Hour", symbol: "mph", toMs: 0.44704, type: "imperial" },
  fts: { name: "Feet / Second", symbol: "ft/s", toMs: 0.3048, type: "imperial" },
  kn: { name: "Knots (Nautical)", symbol: "kn", toMs: 0.514444444, type: "aero" },
  mach: { name: "Mach (Speed of Sound)", symbol: "M", toMs: 343, type: "aero" } // Assume dry air 20C
};

export function SpeedConverter() {
  const [inputValue, setInputValue] = useState<string>("100");
  const [inputUnit, setInputUnit] = useState<UnitOption>("kmh");
  const [precision, setPrecision] = useState<number>(2);

  const baseValueInMs = useMemo(() => {
    const val = parseFloat(inputValue);
    if (isNaN(val)) return 0;
    return val * UNITS[inputUnit].toMs;
  }, [inputValue, inputUnit]);

  const results = useMemo(() => {
    const res: Record<string, string> = {};
    (Object.keys(UNITS) as UnitOption[]).forEach(unit => {
      const val = baseValueInMs / UNITS[unit].toMs;
      res[unit] = parseFloat(val.toFixed(precision)).toString();
    });
    return res;
  }, [baseValueInMs, precision]);

  const exportCSV = () => {
    let csv = `Speed Unit,Value (${results[inputUnit]} ${UNITS[inputUnit].symbol} Equivalent)\n`;
    ...
    a.href = url;
    a.download = `speed-conversion-${results[inputUnit]}${UNITS[inputUnit].symbol}.csv`;
    a.click();
      ...
      <div className="grid lg:grid-cols-[1fr_350px] gap-0 border border-border rounded-[3rem] overflow-hidden bg-background shadow-2xl">
        ...
        <div className="p-6 sm:p-10 space-y-10 border-b lg:border-b-0 lg:border-r border-border">
          ...
          <div className="space-y-6">
                ...
                <div className="absolute right-6 top-1/2 -translate-y-1/2 flex items-center gap-3 bg-background border border-border p-2 pr-6 rounded-2xl shadow-lg w-[200px]">
                   ...
                   >
                     <optgroup label="Metric / SI">
                       <option value="kmh">km/h — Automotive</option>
                       <option value="ms">m/s — Physics (SI)</option>
                     </optgroup>
                     <optgroup label="Imperial">
                       <option value="mph">mph — Automotive</option>
                       <option value="fts">ft/s — Ballistics</option>
                     </optgroup>
                     <optgroup label="Aero / Marine">
                       <option value="kn">kn — Knots</option>
                       <option value="mach">M — Mach No.</option>
                     </optgroup>
                   </select>
                </div>
             </div>
             ...
             <div className="flex items-center gap-4 p-5 rounded-2xl border border-primary/20 bg-primary/5 text-[10px] font-black uppercase tracking-widest text-primary overflow-x-auto whitespace-nowrap hide-scrollbar">
                ...
                <span>{results["kmh"]} km/h</span>
                {inputUnit !== "mph" && (
                  <>
                    <ArrowRightLeft className="w-3 h-3 opacity-50 shrink-0" />
                    <span>{results["mph"]} mph</span>
                  </>
                )}
             </div>
          </div>
        ...
        <div className="p-6 sm:p-10 bg-muted/10 space-y-10 group flex flex-col">
          <div className="space-y-6 flex-1">
             <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground px-2">Scientific Factor</h3>
             ...
             <div className="p-8 rounded-[2.5rem] bg-foreground text-background shadow-2xl relative overflow-hidden transition-all group-hover:shadow-primary/20">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-black italic tracking-tighter truncate" title={UNITS[inputUnit].toMs.toString()}>{UNITS[inputUnit].toMs}</span>
                  </div>
                  <span className="text-sm font-black uppercase">Meters per Second</span>
                </div>
             </div>
             ...
             <div className="space-y-3">
                <div className="p-4 rounded-2xl border border-border bg-background flex items-start gap-4">
                   ...
                   <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Mach Approximation</p>
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Mach No. (M) calculations assume sound traveling through dry air at sea level (20°C / 68°F), moving at approximately 343 m/s.
                      </p>
                   </div>
                </div>
             </div>
...
function ResultCard({ label, symbol, value, type }: { label: string, symbol: string, value: string, type: string }) {
  ...
  if (type === "aero") themeColor = "bg-sky-500";
