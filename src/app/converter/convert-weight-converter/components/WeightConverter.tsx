...
import { 
  Scale, ArrowRightLeft, Download, Info, Link2, Weight
} from "lucide-react";

type UnitOption = "mg" | "g" | "kg" | "mt" | "oz" | "lb" | "st" | "t";

const UNITS: Record<UnitOption, { name: string; symbol: string; toKg: number; type: "metric" | "imperial" }> = {
  mg: { name: "Milligram", symbol: "mg", toKg: 0.000001, type: "metric" },
  g: { name: "Gram", symbol: "g", toKg: 0.001, type: "metric" },
  kg: { name: "Kilogram", symbol: "kg", toKg: 1, type: "metric" },
  mt: { name: "Metric Ton", symbol: "t", toKg: 1000, type: "metric" },
  oz: { name: "Ounce", symbol: "oz", toKg: 0.028349523125, type: "imperial" },
  lb: { name: "Pound", symbol: "lb", toKg: 0.45359237, type: "imperial" },
  st: { name: "Stone", symbol: "st", toKg: 6.35029318, type: "imperial" },
  t: { name: "US Short Ton", symbol: "ton", toKg: 907.18474, type: "imperial" }
};

export function WeightConverter() {
  const [inputValue, setInputValue] = useState<string>("1");
  const [inputUnit, setInputUnit] = useState<UnitOption>("kg");
  const [precision, setPrecision] = useState<number>(4);

  // Core conversion engine
  const baseValueInKg = useMemo(() => {
    const val = parseFloat(inputValue);
    if (isNaN(val)) return 0;
    return val * UNITS[inputUnit].toKg;
  }, [inputValue, inputUnit]);

  const results = useMemo(() => {
    const res: Record<string, string> = {};
    (Object.keys(UNITS) as UnitOption[]).forEach(unit => {
      const val = baseValueInKg / UNITS[unit].toKg;
      // Truncate trailing zeros if it's a clean decimal
      res[unit] = parseFloat(val.toFixed(precision)).toString();
    });
    return res;
  }, [baseValueInKg, precision]);

  const exportCSV = () => {
    let csv = `Weight Unit,Value (${results[inputUnit]} ${UNITS[inputUnit].symbol} Equivalent)\n`;
    ...
    a.href = url;
    a.download = `weight-conversion-${results[inputUnit]}${UNITS[inputUnit].symbol}.csv`;
    a.click();
        ...
        <div className="p-6 sm:p-10 space-y-10 border-b lg:border-b-0 lg:border-r border-border">
          ...
          <div className="space-y-6">
             ...
             <div className="flex items-center gap-4 p-5 rounded-2xl border border-primary/20 bg-primary/5 text-[10px] font-black uppercase tracking-widest text-primary overflow-x-auto whitespace-nowrap hide-scrollbar">
                ...
                <span>{results["kg"]} kg</span>
                {inputUnit !== "lb" && (
                  <>
                    <ArrowRightLeft className="w-3 h-3 opacity-50 shrink-0" />
                    <span>{results["lb"]} lb</span>
                  </>
                )}
             </div>
          </div>
          ...
          <div className="space-y-6 flex-1">
             ...
             <div className="p-8 rounded-[2.5rem] bg-foreground text-background shadow-2xl relative overflow-hidden transition-all group-hover:shadow-primary/20">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-black italic tracking-tighter truncate">{UNITS[inputUnit].toKg}</span>
                    <span className="text-sm font-black uppercase">Kilograms</span>
                  </div>
                </div>
             </div>
             ...
             <div className="space-y-3">
                <div className="p-4 rounded-2xl border border-border bg-background flex items-start gap-4">
                   ...
                   <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Avoirdupois Standard</p>
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        To convert strictly between Imperial systems (e.g. Ounces to Pounds), modern logic translates through the metric {`(${UNITS["kg"].symbol})`} standard defined in 1959.
                      </p>
                   </div>
                </div>
             </div>
...
function ResultCard({ label, symbol, value, type }: { label: string, symbol: string, value: string, type: string }) {
  return (
    <div className="p-4 rounded-2xl border border-border bg-background hover:border-primary/30 hover:shadow-lg transition-all group flex flex-col justify-between min-h-[100px] relative overflow-hidden">
       <div className={`absolute top-0 right-0 w-16 h-16 rounded-full blur-xl -translate-y-1/2 translate-x-1/2 opacity-20 ${type === "metric" ? "bg-emerald-500" : "bg-purple-500"}`} />
