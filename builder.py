...
const fmtPct = (n, d = 2) => (isNaN(n) ? "0" : n.toFixed(d)) + "%";

// Progressive tax on an amount starting from a base income
const calcMarginalTax = (base, additional, brackets, bpa) => {
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#1c1a17", border: "1px solid #3a3020", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#c9b99a" }}>
      <p style={{ fontWeight: 700, color: "#f0d080", marginBottom: 4 }}>{label ? fmtC(Number(label)) : ""}</p>
      {payload.map(p => (
        <p key={p.name} style={{ color: p.color ?? "#c9b99a" }}>{p.name}: {fmtC(p.value)}</p>
      ))}
    </div>
  );
};
...
export default function CapitalGainsTaxCanadaPage() {
  const [proceeds,    setProceeds]    = useState(250000);
  const [acb,         setAcb]         = useState(120000);
  ...
  const [primaryYears,setPrimaryYears]= useState(5);
      ...
      <style>{`
        ...
        .cg-container::before {
          ...
          background-image: repeating-linear-gradient(
            ...
            rgba(200,150,42,0.022) 28px, rgba(200,150,42,0.022) 29px
          );
        }
        ...
        .wf-seg-label { font-size: 0.68rem; font-weight: 800; color: rgba(0,0,0,0.7); letter-spacing: 0.06em; white-space: nowrap; overflow: hidden; font-family: var(--font-b); }
        ...
        .sec-badge {
          ...
          color: var(--amber2); border: 1px solid rgba(200,150,42,0.25);
          font-size: 0.65rem; font-weight: 800; letter-spacing: 0.12em;
        ...
        .cbody .pill {
          ...
          border-radius: 4px; border: 1px solid rgba(200,150,42,0.25); vertical-align: middle;
        }
        ...
        .icard:hover { border-color: rgba(200,150,42,0.4); transform: translateY(-2px); }
        ...
        .alert-box {
          background: rgba(200,150,42,0.08); border: 1px solid rgba(200,150,42,0.25);
        ...
        .cta-btn {
          ...
          background: var(--amber); color: #0f0d08;
              ...
              {calc.rawGain > 0 ? (
                <>
                  <div className="waterfall-bar">
                    {/* Federal tax */}
                    <div className="wf-seg" style={{ width: (calc.rawGain > 0 ? calc.fedTax/calc.rawGain*100 : 0) + "%", background: "#c0402a" }}>
                      {(calc.fedTax/calc.rawGain*100) > 8 && <span className="wf-seg-label">Federal</span>}
                    </div>
                    {/* Provincial tax */}
                    <div className="wf-seg" style={{ width: (calc.rawGain > 0 ? calc.provTax/calc.rawGain*100 : 0) + "%", background: "#b36a20" }}>
                      {(calc.provTax/calc.rawGain*100) > 8 && <span className="wf-seg-label">Prov.</span>}
                    </div>
                    {/* Non-taxable portion (excluded by inclusion rate) */}
                    <div className="wf-seg" style={{ width: (calc.rawGain > 0 ? calc.nonTaxableGain/calc.rawGain*100 : 0) + "%", background: "#2a6644" }}>
                      {(calc.nonTaxableGain/calc.rawGain*100) > 8 && <span className="wf-seg-label">Excluded</span>}
                    </div>
                    {/* Keep (after tax) */}
                    <div className="wf-seg" style={{ width: (calc.rawGain > 0 ? Math.max(0,(calc.rawGain - calc.totalTax - calc.nonTaxableGain))/calc.rawGain*100 : 0) + "%", background: "#1a4a2a" }}>
                      {(Math.max(0,calc.rawGain - calc.totalTax - calc.nonTaxableGain)/calc.rawGain*100) > 10 && <span className="wf-seg-label">Keep</span>}
                    </div>
                  </div>
                  <div className="wf-legend">
                    {[
                      { color: "#c0402a", label: "Federal Tax",        val: fmtC(calc.fedTax),         pct: calc.rawGain>0?calc.fedTax/calc.rawGain*100:0 },
                      { color: "#b36a20", label: "Provincial Tax",     val: fmtC(calc.provTax),        pct: calc.rawGain>0?calc.provTax/calc.rawGain*100:0 },
                      { color: "#2a6644", label: "Excluded (non-taxable)", val: fmtC(calc.nonTaxableGain), pct: calc.rawGain>0?calc.nonTaxableGain/calc.rawGain*100:0 },
                      { color: "#1a4a2a", label: "Net Gain Kept",      val: fmtC(Math.max(0,calc.rawGain-calc.totalTax)), pct: calc.rawGain>0?Math.max(0,calc.rawGain-calc.totalTax)/calc.rawGain*100:0 },
                    ].map(l => (
                    ...
                    <AreaChart data={calc.chartData.filter(d => d.gain <= Math.max(calc.rawGain * 1.5, 300000))} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                      <defs>
                        <linearGradient id="taxGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%"  stopColor="#c0402a" stopOpacity={0.3} />
                          <stop offset="95%" stopColor="#c0402a" stopOpacity={0.03} />
                        </linearGradient>
                        <linearGradient id="keepGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%"  stopColor="#4a9e6a" stopOpacity={0.3} />
                          <stop offset="95%" stopColor="#4a9e6a" stopOpacity={0.03} />
                        </linearGradient>
                      </defs>
                      ...
                      {scenario === "post2024" && entityType === "individual" && <ReferenceLine x={250000} stroke="rgba(200,150,42,0.4)" strokeDasharray="4 4" label={{ value: "$250K", fill: "var(--muted)", fontSize: 10 }} />}
                      <Area type="monotone" dataKey="Tax Owed" stroke="#c0402a" fill="url(#taxGrad)" strokeWidth={2} dot={false} />
                      <Area type="monotone" dataKey="Net Kept" stroke="#4a9e6a" fill="url(#keepGrad)" strokeWidth={2} dot={false} />
...
with open('src/app/finance/capital-gains-tax-canada-calculator/page.tsx', 'w', encoding='utf-8') as f:
    f.write(code)

with open('src/app/finance/capital-gains-tax-canada-calculator/page.tsx', 'r', encoding='utf-8') as f:
    c = f.read()

# Make sure imports for lucide-react and RelatedToolsSection are added
if 'import { TrendingUp' not in c:
    c = c.replace(
        'import { useState, useMemo } from "react";',
        'import { useState, useMemo } from "react";\nimport { TrendingUp, Building2, Home, Bitcoin, Briefcase, Package, Hourglass, TrendingDown, Gift, Landmark, Users, Leaf } from "lucide-react";\nimport { RelatedToolsSection } from "@/components/tools/ToolPageScaffold";'
    )
